...
def generateSyntheticData(cfg):
    ...
    end_num = cfg.numSynthetic - 1
    end_dicom = os.path.join(cfg.imgDir, f'rt_{end_num}.dcm')
    print(f'end dicom {end_dicom}')
    if os.path.exists(end_dicom): # TODO change this name
        print(f'Synthetic data already exists in {cfg.imgDir} and will be reused')
        return

    # Start a separate thread generating the data
    genDataThread = threading.Thread(name='genDataThread',
                                     target=sim.generate_data,
                                     args=(cfg.imgDir, sim_settings))
